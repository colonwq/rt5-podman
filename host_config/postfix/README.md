...
Build the LMDB map databases and reload:
...
Keep Postfix `virtual`/`transport`, exim `aliases`, and `config/RT_SiteConfig.pm` mail addresses in sync. See [docs/add-email-alias.md](../../docs/add-email-alias.md) for adding addresses such as `rt-security@rt.example.com` to a specific RT queue.
...
```bash
sudo postfix check
sudo postmap -q rt@rt.example.com lmdb:/etc/postfix/virtual
sudo postmap -q rt@rt.example.com lmdb:/etc/postfix/transport
```
