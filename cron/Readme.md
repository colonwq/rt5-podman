...
| `backup-attachments.sh` | Tar backup of `/attachments` from `rt5` (keeps 3 newest `attachments-*.tgz` files) |
| `backup-database.sh` | `mysqldump` of `rt5` database from `mariadb` container (keeps 3 newest `rt-*.sql.gz` files) |

Each backup script deletes older files before running, keeping the **2 newest** existing backups so **3** files remain after a successful run. A failed backup that produces a 0-byte file is reported as an error, removed, and the script exits non-zero.
