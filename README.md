...
![RT5 Podman quadlet architecture](docs/architecture.svg)

### Container roles

| Container | Image | Host port | Podman network name | Role |
|-----------|-------|-----------|---------------------|------|
| `mariadb` | `docker.io/mariadb:lts-ubi` | `127.0.0.1:3306` → container `:3306` | `mariadb:3306` | RT database |
| `rt5` | `docker.io/colonwq/rt5:510-5` | `127.0.0.1:8080` → container `:8080` | `rt5:8080` | RT web UI (Apache httpd) |
...
Public users reach RT at `https://rt.example.com` through host Apache, which proxies to `http://127.0.0.1:8080` (loopback only; not published on all host interfaces). Other containers on `rt5-net` use `http://rt5:8080`. See [host_config/apache/README.md](host_config/apache/README.md).
...
| [docs](docs) | [Architecture diagram](docs/architecture.svg), [adding an email alias](docs/add-email-alias.md) |
...
**Apply quadlet health settings to a running system:** copy updated `.container` files, `daemon-reload`, then **restart** each service so Podman creates a new container with `--health-cmd` (a simple `restart` of an old container keeps a container that was created without probes):

```bash
cp /store/rt5-podman/quadlet/*.container ~/.config/containers/systemd/
systemctl --user daemon-reload
systemctl --user restart mariadb.service rt5.service exim.service
```

Confirm the generated unit includes health flags:

```bash
systemctl --user cat mariadb.service | grep -E 'health-cmd|health-interval'
```

Confirm the running container has a healthcheck defined (`null` means the container was not recreated with probes yet):

```bash
podman inspect mariadb --format '{{json .Config.Healthcheck}}'
```

Check probe status (quick summary — `(healthy)` appears only when a healthcheck is configured):

```bash
podman ps --filter name=mariadb --format '{{.Names}}  {{.Status}}'
podman ps --filter name=rt5 --format '{{.Names}}  {{.Status}}'
podman ps --filter name=exim --format '{{.Names}}  {{.Status}}'
```

Run the probe once manually (exit 0 = passed):

```bash
podman healthcheck run mariadb
```

Detailed health state (`null` or `no healthcheck` if probes were never configured — do not use `{{.State.Health.Status}}`; it errors when `State.Health` is absent):

```bash
podman inspect mariadb --format '{{json .State.Health}}' | jq -r '.Status // "no healthcheck"'
podman inspect rt5 --format '{{json .State.Health}}' | jq -r '.Status // "no healthcheck"'
podman inspect exim --format '{{json .State.Health}}' | jq -r '.Status // "no healthcheck"'
```

Or from full JSON:

```bash
podman inspect mariadb | jq -r '.[0].State.Health.Status // "no healthcheck"'
```
...
Host Apache terminates HTTPS and reverse-proxies to the rt5 quadlet on `127.0.0.1:8080` (published on loopback only; container port `8080` on `rt5-net`).
...
| Check | Command |
|-------|---------|
| RT web UI (host loopback) | `curl -sI http://127.0.0.1:8080/` |
